...
### Objective
Build a simple local area network with 2 PCs, 1 Switch, and 1 Router. Configure static IP addressing and verify connectivity using ping.
...
4. Configured IP address on the Router interface and enabled the port
5. Verified connectivity using the `ping` command

### Troubleshooting Notes
- Initially connected PC1 to the Switch Console port by mistake (Console ports are for management only)
- Corrected by reconnecting PC1 to the correct data port (FastEthernet0/3)
- Learned the difference between Console port and network interfaces

### Verification
Successful ping from PC1 to PC0 with 0% packet loss.
...
- Router Interface Configuration (Before enabling the port)
...
- Router Interface Configuration (After enabling the port)
...
- Unsuccessful ping (when PC1 was connected to the console port)
...
- Successful Ping Result after connecting PC1 to the correct port
